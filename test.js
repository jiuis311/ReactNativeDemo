    ...
    it(`generated output must be sorted`, function() {
      expect(result.some(
        (item) => {
          ...
          return item.input.some((number, index) => number !== sortedInput[index])
        }
      )).toBe(false);
    })
    ...
    it(`generated output doesn't contain duplicates`, function() {
      expect(result.some(
        (item) => {
          ...
          return sortedInput.some((number, index) => index < sortedInput.length-1 && number === sortedInput[index+1]);
        }
      )).toBe(false);
    })
